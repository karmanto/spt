...
const TourDetail: React.FC = () => {
  ...
  const fetchTourDetail = async () => {
    if (!slug) {
      navigate('/international-tours');
      return;
    }
    const id = slug.split('-').pop();
    if (!id) {
      setError(t('intlTourNotFound'));
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setError(t('failedToLoadIntlTourDetails'));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!tour) {
    return (
      <div className="min-h-screen flex justify-center items-center bg-gray-50">
        <p className="text-base text-gray-700">{t('intlTourNotFound')}</p>
      </div>
    );
  }
